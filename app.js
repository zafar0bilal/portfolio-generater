const designs = [
["Minimal Light","minimal light","light","Inter"],["Dark Modern","dark modern","dark","Manrope"],["Clean Professional","clean professional","light","DM Sans"],
["Bold Dark","bold dark","dark","Montserrat"],["Pastel Gradient","pastel gradient","creative","Space Grotesk"],["Purple Creative","purple creative","creative","Syne"],
["Beige Editorial","beige editorial","minimal","Libre Baskerville"],["Dark Green","dark green","dark","Manrope"],["Minimal Blue","minimal blue","light","Inter"],
["Creative Dark","creative dark","dark","Syne"],["Modern Agency","modern agency","light","Montserrat"],["Neon Tech","neon tech","dark","Space Grotesk"],
["Photography Minimal","photography minimal","minimal","Playfair Display"],["3D Artist","3d artist","creative","Syne"],["SEO Specialist","seo specialist","light","DM Sans"],
["Corporate","corporate","light","Montserrat"],["Motion Studio","motion studio","dark","Montserrat"],["Tech Dark","tech dark","dark","Space Grotesk"],
["Colorful Fun","colorful fun","creative","Manrope"],["Architecture","architecture","minimal","Playfair Display"],["Mobile Developer","mobile developer","dark","Inter"],
["Life Coach","life coach","light","Playfair Display"],["AI Engineer","ai engineer","dark","Space Grotesk"],["Interior Designer","interior designer","minimal","Montserrat"]
];
...
experience:"2+ years freelance experience\nWorked with startups and small businesses",education:"BS Computer Science — Your University",resume:"",photo:"",template:0,
projects:[
{name:"Creative Landing Page",type:"Web Design",description:"A clean conversion-focused landing page for a growing digital brand.",image:""},
{name:"Brand Identity",type:"Branding",description:"A complete visual identity system with logo, colors and social assets.",image:""},
{name:"Mobile App UI",type:"UI/UX",description:"A modern mobile experience designed around clarity and simple interactions.",image:""}
]};
...
function renderProjects(){
 ...
 state.projects.forEach((p,i)=>{
  ...
  <div class="project-image-upload">
    <div class="project-image-preview" id="projectPreview${i}">${p.image?`<img src="${p.image}" alt="">`:"WORK PIC"}</div>
...
$("addProject").onclick=()=>{if(state.projects.length>=6)return alert("You can add up to 6 projects.");state.projects.push({name:"New Project",type:"Design",description:"Describe your project here.",image:""});renderProjects();save();renderPreview()};
...
function renderDesigns(){
 ...
 designs.forEach((d,i)=>{
  ...
  card.innerHTML=`<div class="design-thumb t${i+1}"><span class="design-num">${String(i+1).padStart(2,"0")}</span><div class="mock-nav"></div><div class="mock-title"></div><div class="mock-line"></div><div class="mock-block"></div></div><div class="design-name">${d[0]}<div class="design-tag">${d[2].toUpperCase()}</div><div class="font-note">Aa ${d[3]}</div></div>`;
...
function renderPreview(){
 const i=state.template,d=designs[i],skills=splitList(state.skills);
 $("previewTemplate").textContent=`Template ${String(i+1).padStart(2,"0")} · ${d[0]} · ${d[3]}`;
 $("browserTitle").textContent=`${state.name||"Portfolio"} — ${state.title||"Freelancer"}`;
 $("livePreview").innerHTML=`<div class="site t${i+1}">
 ...
 <section class="site-section" id="work"><h3>Selected Work</h3><div class="project-grid">${state.projects.map((p,n)=>`<article class="project"><div class="project-img">${p.image?`<img src="${p.image}" alt="${esc(p.name)}">`:`0${n+1}`}</div><div class="project-body"><strong>${esc(p.name)}</strong><p>${esc(p.type)} · ${esc(p.description)}</p></div></article>`).join("")}</div></section>
...
function updateReady(){$("readyName").textContent=state.name||"Your portfolio";$("readyTemplate").textContent=`Template ${String(state.template+1).padStart(2,"0")} — ${designs[state.template][0]} · ${designs[state.template][3]}`}
function go(step){
