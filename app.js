const designs = [
  ["Crimson Editorial","bold red editorial","dark","Montserrat","assets/presets/preset-01.png","reveal"],
  ["Warm Luxury","warm luxury","dark","Playfair Display","assets/presets/preset-02.png","parallax"],
  ["Golden Showcase","premium editorial","dark","Libre Baskerville","assets/presets/preset-03.png","stagger"],
  ["Neon Arena","gaming dashboard","dark","Space Grotesk","assets/presets/preset-04.png","glitch"],
  ["Glass UI","futuristic glass","dark","Manrope","assets/presets/preset-05.png","float"],
  ["Blue Grid","graphic grid","light","Montserrat","assets/presets/preset-06.png","wipe"]
];
...
function renderDesigns(){
 ...
 designs.forEach((d,i)=>{
  ...
  card.innerHTML=`<div class="design-thumb has-ref t${i+1}" style="--ref:url('${d[4]}')"><span class="design-num">${String(i+1).padStart(2,"0")}</span><div class="design-overlay"><b>${d[0]}</b><span>${d[5].toUpperCase()}</span></div></div><div class="design-name">${d[0]}<div class="design-tag">${d[2].toUpperCase()} · PRESET ${i+1}</div><div class="font-note">Aa ${d[3]}</div></div>`;
  card.onclick=()=>{state.template=i;save();renderDesigns();renderPreview();updateReady();if(document.getElementById("animationOverride"))document.getElementById("animationOverride").value="auto";};
...
function renderPreview(){
 const i=state.template,d=designs[i],skills=splitList(state.skills),anim=d[5],preset=i+1;
 $("previewTemplate").textContent=`Template ${String(i+1).padStart(2,"0")} · ${d[0]} · ${d[3]} · ${d[5]}`;
 $("browserTitle").textContent=`${state.name||"Portfolio"} — ${state.title||"Freelancer"}`;
 $("livePreview").innerHTML=`<div class="site t${i+1} preset-${preset}" data-anim="${anim}">
...
document.getElementById("animationOverride")?.addEventListener("change",e=>{load();
